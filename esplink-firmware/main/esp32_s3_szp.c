...
// 设置液晶屏颜色（优化版本 - 使用更大的缓冲区）
void lcd_set_color(uint16_t color)
{
    // 分配一个更大的缓冲区 - 一次绘制多行以提高效率
    const int lines_per_batch = 10;  // 每次绘制10行
    const size_t buffer_size = BSP_LCD_H_RES * lines_per_batch * sizeof(uint16_t);

    uint16_t *buffer = (uint16_t *)heap_caps_malloc(buffer_size, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);

    if (NULL == buffer)
    {
        ESP_LOGE(TAG, "Memory allocation failed for lcd_set_color");
        return;
    }

    // 填充缓冲区
    for (size_t i = 0; i < BSP_LCD_H_RES * lines_per_batch; i++)
    {
        buffer[i] = color;
    }

    // 批量绘制，每次10行
    for (int y = 0; y < BSP_LCD_V_RES; y += lines_per_batch)
    {
        int remaining_lines = BSP_LCD_V_RES - y;
        int lines_to_draw = (remaining_lines < lines_per_batch) ? remaining_lines : lines_per_batch;

        esp_lcd_panel_draw_bitmap(panel_handle, 0, y, BSP_LCD_H_RES, y + lines_to_draw, buffer);
    }
...
{
    if (x >= 0 && x < BSP_LCD_H_RES && y >= 0 && y < BSP_LCD_V_RES) {
...
{
    lcd_draw_line(x, y, x + w, y, color);         // 上边
    lcd_draw_line(x, y, x, y + h, color);         // 左边
    lcd_draw_line(x + w, y, x + w, y + h, color); // 右边
    lcd_draw_line(x, y + h, x + w, y + h, color); // 下边
}
...
void lcd_fill_rect(int x, int y, int w, int h, uint16_t color)
{
    // 边界检查
    if (w <= 0 || h <= 0) return;
    if (x < 0 || y < 0 || x >= BSP_LCD_H_RES || y >= BSP_LCD_V_RES) return;
    ...
    int buffer_width = w;
    int buffer_lines = (h < lines_per_batch) ? h : lines_per_batch;

    uint16_t *buffer = (uint16_t *)heap_caps_malloc(
        buffer_width * buffer_lines * sizeof(uint16_t),
        MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL
    );

    if (NULL == buffer)
    {
        ESP_LOGE(TAG, "Memory allocation failed for lcd_fill_rect");
        return;
    }

    // 填充缓冲区
    for (int i = 0; i < buffer_width * buffer_lines; i++)
    {
        buffer[i] = color;
    }
