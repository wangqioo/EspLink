...
截至 2026-06-20，当前开发计划内的主体代码和本地/真机核心验证已经完成。项目已经具备：

- 微信小程序 BLE 扫描、BluFi 配网、设备上线等待和绑定；
- ESP32-S3 固件 WiFi/NVS/恢复出厂、启动注册、WebSocket 长连接和 OTA 客户端；
- Node 后端设备注册、绑定、WebSocket、AI 代理、设备在线状态和 OTA 决策；
- 管理后台固件上传、发布、停用、重复版本提示、OTA 结果汇总和 OTA check 预览；
- 生产 PSK 签名注册、签名 OTA result、生产传输 URL 校验和 ESP-IDF crt bundle 接入；
- OTA 正常升级、同版本强制升级、wrong SHA 拒绝、下载中断恢复、boot-fail 自动回滚真机验证；
- 后端重启后的固件 WebSocket 断线重连真机验证。

仍未关闭的是上线环境或外部工具依赖项：

- 真实域名 HTTPS/WSS、证书和反向代理链路验证；
- 微信开发者工具和 iOS 真机上的 BLE/配网页面复测；
- 路由器断开、长时间在线、弱网恢复等稳定性观察窗口；
- 可选后续产品功能：二维码快速配网、手机端推送通知、多芯片支持、远程诊断。

| 模块 | 状态 | 说明 |
|------|------|------|
| 后端 API / WebSocket | 已完成 | 注册、绑定、设备长连接、AI 代理、限流、OTA 决策和结果记录已接入 |
| 管理后台 | 已完成 | 固件上传发布、旧版本停用、OTA 预览和 OTA 结果摘要已接入 |
| ESP32-S3 固件 | 已完成核心闭环 | 配网、NVS、启动注册、WebSocket、OTA、安全签名和回滚路径已验证 |
| 微信小程序 | 已完成静态和主要流程 | BLE/BluFi/绑定/设备列表已实现，仍需微信开发者工具和 iOS 真机复测 |
| 生产安全 | 代码准备完成 | PSK 签名、nonce/timestamp、防重放和 HTTPS/WSS 强制策略已接入 |
| 上线环境 | 待部署验证 | 需要真实域名、证书、反向代理和生产 `.env` |
...
| 验证项 | 状态 | 最近证据 |
|--------|------|----------|
| 后端测试 | 已通过 | `npm test -- --runInBand`，详见开发状态文档 |
| 管理后台构建 | 已通过 | `backend/admin-frontend npm run build` |
| 小程序静态测试 | 已通过 | `node --test tests/*.test.js` |
| 默认固件构建 | 已通过 | `idf.py build`，默认配置关闭本地 WiFi 注入和生产强制签名 |
| 签名启动注册 | 已通过真机验证 | `/dev/cu.usbmodem112301`，MAC `10:51:DB:80:E2:E8` |
| 签名 OTA result | 已通过真机验证 | wrong SHA 发布验证时后端接受 signed result |
| 普通 OTA / 强制 OTA | 已通过真机验证 | `1.0.3 -> 1.0.4 -> 1.0.5` 路径 |
| wrong SHA 拒绝 | 已通过真机验证 | 固件上报 `sha_mismatch`，不切换不可信镜像 |
| 下载中断恢复 | 已通过真机验证 | 固件上报 `download_failed` 并恢复上一有效固件 |
| boot-fail OTA 回滚 | 已通过真机验证 | 临时 `1.0.8` 崩溃镜像回滚到 `1.0.5` |
| 后端重启 / WebSocket 恢复 | 已通过真机验证 | 后端重启后同一 MAC 重新连接 |
| HTTPS/WSS 真域名 | 待部署验证 | 依赖真实域名、证书和反向代理 |
